...
def _payload(result) -> dict:
    """Extract a tool's dict payload from a CallToolResult, the same way
    ACEL's own middleware does: prefer structured_content, else parse the
    JSON text block the SDK serializes plain-dict returns into."""
...
def test_shadow_mode_logs_but_never_blocks_over_the_real_proxy():
    """The live equivalent of the shadow-mode Session tests: same violating
    sequence as test_temporal_violation_halts_delete_before_validate, but
    with the toy server built in shadow mode — the call must now succeed."""
    server, acel_session = build_server(mode="shadow")
    assert acel_session.mode == "shadow"

    async def scenario(client: ClientSession):
        result = await client.call_tool("delete_record", {"record_id": "r_42"})
        assert result.is_error is not True  # NOT blocked, unlike enforce mode
        assert _payload(result) == {"deleted": True, "record_id": "r_42"}

    run(_run_client(server, scenario))
    # The call really happened (shadow mode lets it through)...
    assert [row["tool"] for row in acel_session.trace] == ["delete_record"]
    # ...but ACEL still detected and recorded that it broke the rule.
    assert len(acel_session.violations) == 1
    assert acel_session.violations[0].kind == "temporal"
    assert acel_session.evidence.verify() is True
